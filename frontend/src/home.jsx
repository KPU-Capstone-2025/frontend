...
import section2 from "./assets/색션2.jpg";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const isDesktop = window.matchMedia("(min-width: 64rem)").matches; // 1024px+
    if (!isDesktop) return;

    const LOCK_MS = 900; 
    const THRESHOLD = 10; // 작은 트랙패드 흔들림 무시
    ...
    // 터치패드/매직마우스도 wheel로 들어옴
    window.addEventListener("wheel", onWheel, { passive: false });
    window.addEventListener("keydown", onKeyDown);

    // 첫 진입 시 현재 active 섹션으로 스냅
    scrollTo(active);
  ...
  const onTouchEnd = (e) => {
    ...
    if (isDesktop) return; // 데스크탑은 wheel로만
...
<section className="fp-section sec3" ref={(el) => (sectionRefs.current[2] = el)}>
  <div className="fp-inner sec">
    <div className="sec__head">
      ...
      <p className="sec__sub">
        수집 → 분석 → 알림 → 조치(가이드)까지 한 화면에서 연결합니다.
      </p>
    </div>
    ...
    <div className="arch">
      ...
      {/* (선택) 우측 점 네비게이션처럼 만들고 싶으면 추가 해야댐 */}
    </div>
  );
}
